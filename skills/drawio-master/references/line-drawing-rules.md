...
## PART 17: WAYPOINT COORDINATE SYSTEM (Critical — AI fails repeatedly)
...
**Example** — icon `i-bucket-security` nested 3 levels:
```
...
|---|---|---|
| Single L→R edge, clear corridor, no obstacles | ❌ NO | Auto-route handles L-shape correctly |
| Fan-in: 2+ sources at same Y → same target | ✅ YES | Prevent overlap (offset lanes) |
| Intermediate container blocking direct path | ✅ YES | Bypass around obstacle |
| Parallel edges same direction same corridor | ✅ YES | Lane offset prevents overlap |
| Sources at DIFFERENT Y (>50px apart) | ❌ NO | Stagger entryY alone is sufficient |

### Anti-Patterns (from failed attempts)

#### ❌ AP-W1: Waypoints with near-zero Y difference

```xml
<!-- WRONG — waypoint Y only 1px from source exit → causes jitter -->
<Array as="points">
  <mxPoint x="660" y="404" />
  <mxPoint x="660" y="403" />
</Array>
...
**Fix**: If waypoint Y ≈ source exit Y (within 20px), REMOVE the waypoint. Use auto-route.

#### ❌ AP-W2: Using relative coordinates for waypoints

```xml
<!-- WRONG — icon is at relative x=172 inside nested container at absolute x=1072 -->
<!-- But waypoint uses 172 instead of 1072 -->
<Array as="points">
  <mxPoint x="172" y="200" />  <!-- ← RELATIVE value, not absolute! -->
</Array>
```

**Fix**: Always sum parent containers to get absolute coordinates.

#### ❌ AP-W3: Waypoints inside container boundaries

```xml
<!-- WRONG — waypoint at x=900 is INSIDE Log Archive container (x=860, width=500) -->
<Array as="points">
  <mxPoint x="900" y="400" />
</Array>
```
...
```
Layout:
  InfoSec Account: x=40, y=310, w=560, h=200  → right edge at x=600
  Log Archive:     x=860, y=40, w=500, h=740  → left edge at x=860
...
Target (bucket-security): parent=c-kms-scope→c-region→c-logarchive
...
Waypoints for GuardDuty (lane 0):
  ...
  wp1 = (660, 404)   ← at source exit Y
  wp2 = (660, 333)   ← at target entry Y (entryY=0.3 → 310 + 78*0.3 = 333)

Waypoints for Firehose (lane 1, +20px offset):
  corridor_x = 680
  wp1 = (680, 404)   ← at source exit Y
  wp2 = (680, 365)   ← at target entry Y (entryY=0.7 → 310 + 78*0.7 = 365)
```

### Pre-XML Validation Checklist for Waypoints

```
□ Computed ABSOLUTE position of every source/target icon (sum all parents)
□ Waypoint X values fall within corridor (between containers, not inside any)
□ Waypoint Y values differ by >20px from source exit Y (otherwise remove waypoint)
□ No two parallel edges share same waypoint X (offset ≥20px per lane)
